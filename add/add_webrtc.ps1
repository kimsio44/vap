# add_webrtc.ps1
# Phase 2.3 — WebRTC Peer Connection
# Lance depuis C:\Users\kimsd\OneDrive\Bureau\vap\

Write-Host "==> Phase 2.3 - WebRTC Manager..." -ForegroundColor Cyan

# ---------------------------------------------------------------------------
# 1. Ajouter webrtc aux dependances workspace
# ---------------------------------------------------------------------------
$workspaceCargo = Get-Content "Cargo.toml" -Raw
if ($workspaceCargo -notmatch "webrtc\s*=") {
...
bytes = "1"
'@
    Write-Host "  [OK] webrtc ajoute au workspace Cargo.toml" -ForegroundColor Green
} else {
    Write-Host "  [SKIP] webrtc deja present" -ForegroundColor Yellow
}

# ---------------------------------------------------------------------------
# 2. Ajouter webrtc aux deps du crate core
# ---------------------------------------------------------------------------
$coreCargo = Get-Content "crates\core\Cargo.toml" -Raw
if ($coreCargo -notmatch "webrtc\s*=") {
    Add-Content "crates\core\Cargo.toml" @'

webrtc = { workspace = true }
bytes = { workspace = true }
'@
    Write-Host "  [OK] webrtc ajoute au crate core" -ForegroundColor Green
} else {
    Write-Host "  [SKIP] webrtc deja present dans core" -ForegroundColor Yellow
}

# ---------------------------------------------------------------------------
# 3. webrtc_manager.rs
# ---------------------------------------------------------------------------
[System.IO.File]::WriteAllText("$PWD\crates\core\src\webrtc_manager.rs", @'
// crates/core/src/webrtc_manager.rs
//
// WebRTC Manager — gere la PeerConnection entre hote et client.
//
// Roles selon le peer :
...
// ---------------------------------------------------------------------------
// WebRtcManager — cree et gere la PeerConnection
// ---------------------------------------------------------------------------
...
impl WebRtcManager {
        ...
        let data_channel = self.peer_connection
            .create_data_channel(
                ...
                Some(RTCDataChannelInit {
                    ordered: Some(false),      // Non-ordonné pour les inputs (latence < fiabilite)
    ...
    pub async fn send_video_frame(&self, frame: &EncodedFrame) -> Result<(), WebRtcError> {
        let track = self.video_track.as_ref()
            .ok_or_else(|| WebRtcError::Internal("No video track — call setup_as_host first".into()))?;
...
// ---------------------------------------------------------------------------
// InputEvent — messages clavier/souris envoyes par le client via DataChannel
// ---------------------------------------------------------------------------
...
'@)
Write-Host "  [OK] webrtc_manager.rs" -ForegroundColor Green

# ---------------------------------------------------------------------------
# 4. Compilation (webrtc est gros, ca va prendre 2-3 minutes)
# ---------------------------------------------------------------------------
Write-Host ""
Write-Host "==> Compilation (webrtc-rs est volumineux, ~2-3 min)..." -ForegroundColor Cyan
Write-Host "    Patience ! C'est normal." -ForegroundColor Yellow
cargo build -p remotedesk-core 2>&1

Write-Host ""
Write-Host "==> Phase 2.3 terminee !" -ForegroundColor Green
Write-Host "    Pipeline complet : DXGI -> H264 -> WebRTC P2P" -ForegroundColor Yellow
