...
fn capture_loop_windows(
    ...
    use windows::Win32::Graphics::Direct3D11::{
        ...
        D3D11_MAP_READ, ID3D11Texture2D,
    };
    ...
    unsafe {
        D3D11CreateDevice(
            ...
            Default::default(), None, 11,
            Some(&mut d3d_device), None, Some(&mut d3d_context),
    ...
    let factory: IDXGIFactory1 = unsafe {
        CreateDXGIFactory1().map_err(|e| CaptureError::DxgiInit(e.to_string()))?
    };
    let adapter = unsafe {
        factory.EnumAdapters1(0).map_err(|e| CaptureError::DxgiInit(format!("No adapter: {}", e)))?
    };
    let output: IDXGIOutput = unsafe {
        adapter.EnumOutputs(config.monitor_index)
            .map_err(|e| CaptureError::DxgiInit(format!("Monitor {} not found: {}", config.monitor_index, e)))?
    };
    let output1: IDXGIOutput1 = output.cast()
        .map_err(|e| CaptureError::DxgiInit(e.to_string()))?;

    // 3. Desktop Duplication (on recupere les dimensions via DXGI_OUTDUPL_DESC)
    let duplication = unsafe {
        output1.DuplicateOutput(&device)
            .map_err(|e| CaptureError::DxgiInit(format!("DuplicateOutput: {}", e)))?
    };

    // GetDesc depuis la duplication â€” contient ModeDesc avec Width/Height
    let dupl_desc = unsafe {
        duplication.GetDesc()
    };
    let src_width  = dupl_desc.ModeDesc.Width;
    let src_height = dupl_desc.ModeDesc.Height;
    ...
    let staging = staging_opt.ok_or_else(|| CaptureError::DxgiInit("CreateTexture2D None".into()))?;
...
fn capture_loop_stub(
    ...
    warn!("DXGI not available Ã¢â‚¬â€ using stub (black frames)");
